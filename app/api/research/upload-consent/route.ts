...
export async function POST(request: Request) {
  try {
    ...
    const fileName = participantId
      ? `consent-${studyId}-${participantId}-${timestamp}.${fileExtension}`
      : `consent-${studyId}-${timestamp}.${fileExtension}`

    // Upload to Supabase Storage
    const bucketName = "research-consents" // You may need to create this bucket in Supabase
    ...
    console.error("Unexpected error in POST /api/research/upload-consent:", error)
    return NextResponse.json(
      {
