 ...
 * @returns True if valid (not in the future), false otherwise
 */
export function validateDateOfBirth(dateOfBirth: string): boolean {
  ...
  const dob = new Date(dateOfBirth + 'T00:00:00'); // Add time to avoid timezone issues
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  // Allow dates up to and including today (not in the future)
  return dob <= today;
}
